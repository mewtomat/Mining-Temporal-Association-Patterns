...
int main(int argc, char* argv[]){
	...
	for(int i=0;i<TAPSeq.size();++i){
		for(int j=i+1;j<TAPSeq.size();++j){
			if(TAPEpoch[i] == TAPEpoch[j]){
				if(isPrefix(TAPSeq[i], TAPSeq[j])) longestTap[i] = false;
			}
			if((TAPEpoch[i]+TAPSeq[i].size()-1 == TAPEpoch[j]+TAPSeq[j].size()-1) &&
				(TAPfreq[i] == TAPfreq[j])){
				if(isSuffix(TAPSeq[i], TAPSeq[j])) longestTap[i] = false;
			}
		}	
	}
	int numFinalTAPs=0;
	...
	for(int i=0;i<TAPSeq.size();++i){
		...
		// cout<<"support set for sequence "<<i+1<<" is of size "<<TAPSupport[i].size()<<endl;
