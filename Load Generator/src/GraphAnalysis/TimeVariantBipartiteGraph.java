...
public class TimeVariantBipartiteGraph {
	...
	public final int EPOCH_LENGTH = 4000;
	...
	public TimeVariantBipartiteGraph(String path, String fileName) {
	...
	public void ConstructGraph () {
		...
		// System.out.println("Max Edges in a quantum:" + maxEdgeCounter + ", totalEdges:" + totalEdges);
	}
	...
	public void publishPatternsChrom() {
		...
		Integer counterKey;
		Hashtable<String, Integer> patternCounter = new Hashtable <String, Integer>();
			...
			for (int j=0;j<keySet.size();j++) {
				...
				if (tokens.length >=MIN_PATTERN_LENGTH) {
					...
					if (edges.size() >= PATTERN_SUPPORT) {
						...
						if (counterKey == null) {
							patternCounter.put(someKey, new Integer (1));
						}
						...
						pr.println(nextKey.replaceAll("RED","1").replaceAll("YELLOW","2").replaceAll("BLUE","3").replaceAll("GREEN","4").replaceAll("PURPULE","5").replaceAll("BROWN","6").replaceAll("BLACK","7").replaceAll("WHITE","8").replaceAll("ORANGE","9").replaceAll("GRAY","10").replaceAll("_"," ") + ", Epoch:" + i+", Support: "+edges.size());
						// pr.println("----------------------");
						// it1 = edges.iterator();
						// for (int k =0;k<edges.size();k++) {
						// 	pr.print(it1.next() + ", ");
	...
	public static void main(String[] args) {
		...
		TimeVariantBipartiteGraph tvbg = new TimeVariantBipartiteGraph(args[0], args[1]);
