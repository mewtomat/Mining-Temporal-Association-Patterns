...
public class TimeVariantBipartiteGraph_RealData_New {
	...
	public TimeVariantBipartiteGraph_RealData_New(String path, String fileName) {
		...
		String outputPath = path + "output_ShortPattern_Names_" + fileTokens[0] + "_" +  PATTERN_SUPPORT + "_" + MIN_PATTERN_LENGTH + "_" + EPOCH_LENGTH + "_" + NoOfCOLORS + ".txt";
		String line;
		String[] tokens;
		try {
	...
	public void ConstructGraph () {
		...
		// Set<String> keySet = currentPatterns.keySet(); 			//MADE CHANGES
		...
		try {
			while ( (line = br.readLine()) != null) {
				...
				//if (line.indexOf("----------") < 0) {
					...
					if (tokens.length >= 3) {
						lhsNode = tokens[0].trim();
						rhsNode = tokens[2].trim();
						edge = lhsNode + "_" + rhsNode;
						oldKey = findKey (edge);
						for (int i=0;i<NoOfCOLORS;i++) {
							if (ColorThresholds[i] > Integer.parseInt(tokens[1].trim())) {
								color = i;
								key = Colors[i-1];
								break;
							}
						...
						if (keySet.contains(key)) {
							edgesSet = (HashSet<String>)currentPatterns.get(key);
							edgesSet.add(edge);
						}
						else {
		...
		System.out.println("Max Edges in a quantum:" + maxEdgeCounter + ", totalEdges:" + totalEdges + ", total quantums:" + quantumCounter);
	}
	...
	public void cleanupPatterns (Hashtable<String, HashSet<String>> currentPatterns) {
		...
		Set keySet = currentPatterns.keySet();				
		// Set<String> keySet = currentPatterns.keySet();				//MADE CHANGE
	...
	public void publishPatternsChrom() {
			...
			for (int j=0;j<keySet.size();j++) {
				...
				if (tokens.length >=MIN_PATTERN_LENGTH) {
					...
					if (edges.size() >= PATTERN_SUPPORT) {
						...
						counterKey = patternCounter.get(someKey);
		...
		printPatternCounter(patternCounter);
	...
	private void printPatternCounter(Hashtable<String, PatternStat>  patternCounter) {
		
		double avgSupport;
		
		int count = patternCounter.size();
		
		// Set<String> keySet = patternCounter.keySet();  					//MADE CHANGE
	...
	public static void main(String[] args) {
		...
		//TimeVarientBipartiteGraph_RealData tvbg = new TimeVarientBipartiteGraph_RealData("D:\\manoj\\3gBI\\Novartis\\Paper\\data\\", "dummyData_2_Dense_20_20_4000_50_0.1.txt");
		
		TimeVariantBipartiteGraph_RealData_New tvbg = new TimeVariantBipartiteGraph_RealData_New(args[0], args[1]);
